...
import org.eclipse.viatra.cep.core.api.IActionHandler
import org.eclipse.viatra.cep.core.api.ICepAdapter
...
class VeplJvmModelInferrer extends AbstractModelInferrer {
	...
	def dispatch void infer(EventModel element, IJvmDeclaredTypeAcceptor acceptor, boolean isPreIndexingPhase) {
		...
		//		iqPatterns.generateIncQuery2ViatraCep(element, acceptor)	//this should be added again to support IQ
						...
						body = [
							append(
								...
								'''«referClass(it, p, EventDrivenTransformationBuilder, model.newTypeRef(match),
									model.newTypeRef(matcher))»''').append('''builder = new ''').append(
								'''«referClass(it, p, EventDrivenTransformationRuleFactory)»''').append(
								'''().createRule();
									''').append(
								'''		builder.addLifeCycle(EventDrivenTransformationRuleFactory.INTERVAL_SEMANTICS);
									''').append(
								'''		builder.precondition(«matcher».querySpecification());
									''')
							for (eventPattern : groupedPatterns.get(p)) {
								append(
									...
									'''«referClass(it, eventPattern, IMatchProcessor, model.newTypeRef(match))»() {
										''').append(
									'''		public void process(final «match» matchedPattern) {
										''').append('''				«eventPattern.getFqn(AtomicPatternFqnPurpose.EVENT)»''').append(
									''' event = new «eventPattern.getFqn(AtomicPatternFqnPurpose.EVENT)»(null);
										''').append(
									'''
							...
							append(
								...
								'''«referClass(it, p, IncQueryException)» e) {
									e.printStackTrace();
								} catch (''').append('''«referClass(it, p, InconsistentEventSemanticsException)»''').append(
								''' e) {
									e.printStackTrace();
								}
								return null;
	...
	def private createOrdering(ITreeAppendable appendable, EObject ctx, ComplexEventExpression expression) {
		appendable.append('''setOperator(''').append('''«referClass(appendable, ctx, EventsFactory)».eINSTANCE''').
			append('''.«getOperator(expression)»''').append(
				''');
					''')
	}
