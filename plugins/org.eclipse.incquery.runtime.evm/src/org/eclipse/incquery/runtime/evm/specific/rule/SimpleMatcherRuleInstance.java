...
public class SimpleMatcherRuleInstance<Match extends IPatternMatch> extends RuleInstance {
     ...
     */
    protected SimpleMatcherRuleInstance(final SimpleMatcherRuleSpecification<Match> specification, final Match filter) {
        super(specification);
        throw new UnsupportedOperationException("Do not use this subclass of rule instance!");
    }
...
//            
//            // check filter (this might be expensive!!!)
//            if(!getFilter().isCompatibleWith(atom)) {
//                return;
//            }
//            
//            Map<IncQueryActivationStateEnum, Activation> column = getActivations().column(atom);
//            if(column.size() > 0) {
...
//            if(getSpecification().getLifeCycle().containsTo(IncQueryActivationStateEnum.UPDATED)) {
