 ...
 *******************************************************************************/
package org.eclipse.viatra.integration.xcore.validation;
...
public class ViatraQueryXcoreJavaValidator extends AbstractViatraQueryXcoreJavaValidator {
    
    @Inject
    private IQualifiedNameProvider qualifiedNameProvider;
    
	@Check
	public void checkViatraQueryDerivedFeature(XViatraQueryDerivedFeature feature) {
	    if (feature.getPattern() == null) {
	        this.acceptError("The pattern reference must not be null!", feature, XcorePackage.Literals.XVIATRA_QUERY_DERIVED_FEATURE__PATTERN, -1, null);
	    }
	    else if (feature.getPattern().getParameters().size() != 2) {
	        this.acceptError("The referenced pattern must have exactly two parameters!", feature, XcorePackage.Literals.XVIATRA_QUERY_DERIVED_FEATURE__PATTERN, -1, null);
	    }
	    else {
	        ...
	        if (!(p1TypeName.toString().equals(classTypeName.toString()) && ((p2TypeName == null) || (p2TypeName.toString().equals(featureTypeName.toString()))))) {
	            this.acceptError("A pattern with parameters ("+classTypeName.toString()+", "+featureTypeName.toString()+") can be used in this context!", feature, XcorePackage.Literals.XVIATRA_QUERY_DERIVED_FEATURE__PATTERN, -1, null);
	        }
	    }
	}
}
