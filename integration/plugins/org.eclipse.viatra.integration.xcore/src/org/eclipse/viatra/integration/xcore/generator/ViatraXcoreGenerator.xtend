 ...
 * on the definition of the Viatra Query & Xcore metamodel. Apart from
 ...
 * "org.eclipse.viatra.query.querybasedfeature". <br>
 ...
 * 
 */
class ViatraXcoreGenerator extends XcoreGenerator {

	@Inject
	XbaseCompiler compiler

	@Inject
	private extension ViatraQueryXcoreMapper mappings

	public static String queryBasedFeatureFactory = QueryBasedFeatures.ANNOTATION_SOURCE
			...
			if (xClassifier instanceof XDataType) {
				...
				for (eStructuralFeature : eClass.EAllStructuralFeatures) {
					if (processed.add(eStructuralFeature)) {
						...
						if (xFeature != null) {
							if (xFeature instanceof XViatraQueryDerivedFeature) {
								val XViatraQueryDerivedFeature feature = xFeature as XViatraQueryDerivedFeature
