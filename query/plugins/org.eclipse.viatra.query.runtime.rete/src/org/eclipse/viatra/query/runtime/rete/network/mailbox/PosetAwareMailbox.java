...
public class PosetAwareMailbox extends AbstractUpdateSplittingMailbox<GroupBasedMessageIndexer, PosetAwareReceiver> {
    ...
    public void postMessage(final Direction direction, final Tuple update) {
        ...
        if (antiMonotoneQueue.isEmpty()) {
            this.group.notifyLostAllMessages(this, MessageKind.ANTI_MONOTONE);
        } else {
            this.group.notifyHasMessage(this, MessageKind.ANTI_MONOTONE);
        }

        if (monotoneQueue.isEmpty()) {
            this.group.notifyLostAllMessages(this, MessageKind.MONOTONE);
        } else {
            this.group.notifyHasMessage(this, MessageKind.MONOTONE);
        }
    }
