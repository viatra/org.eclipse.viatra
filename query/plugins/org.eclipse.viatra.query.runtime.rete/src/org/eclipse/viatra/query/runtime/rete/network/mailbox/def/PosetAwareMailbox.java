...
public class PosetAwareMailbox extends AbstractUpdateSplittingMailbox<GroupBasedMessageIndexer, PosetAwareReceiver> {
    ...
    public void postMessage(final Direction direction, final Tuple update, final DifferentialTimestamp timestamp) {
        ...
        if (antiMonotoneQueue.isEmpty()) {
            this.group.notifyLostAllMessages(this, DefaultSelector.ANTI_MONOTONE);
        } else {
            this.group.notifyHasMessage(this, DefaultSelector.ANTI_MONOTONE);
        }

        if (monotoneQueue.isEmpty()) {
            this.group.notifyLostAllMessages(this, DefaultSelector.MONOTONE);
        } else {
            this.group.notifyHasMessage(this, DefaultSelector.MONOTONE);
        }
    }
        ...
        if (kind == DefaultSelector.ANTI_MONOTONE) {
            ...
            for (final Tuple group : this.antiMonotoneQueue.getGroups()) {
                for (final Entry<Tuple, Integer> entry : this.antiMonotoneQueue.getTuplesByGroup(group).entrySet()) {
                    ...
                    for (int i = 0; i < Math.abs(count); i++) {
                        this.receiver.updateWithPosetInfo(Direction.REVOKE, update, false, null);
                    }
                }
            }
            ...
            for (final Tuple group : this.monotoneQueue.getGroups()) {
                for (final Entry<Tuple, Integer> entry : this.monotoneQueue.getTuplesByGroup(group).entrySet()) {
                    ...
                    for (int i = 0; i < Math.abs(count); i++) {
                        this.receiver.updateWithPosetInfo(direction, update, true, null);
                    }
                }
            }
