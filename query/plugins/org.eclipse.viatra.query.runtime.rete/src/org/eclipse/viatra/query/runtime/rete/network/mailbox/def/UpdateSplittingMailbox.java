...
public class UpdateSplittingMailbox extends AbstractUpdateSplittingMailbox<DefaultMessageIndexer, Receiver>
    ...
    public void postMessage(final Direction direction, final Tuple update, final DifferentialTimestamp timestamp) {
        ...
        if (antiMonotoneQueue.isEmpty()) {
            targetGroup.notifyLostAllMessages(targetMailbox, DefaultSelector.ANTI_MONOTONE);
        } else {
            targetGroup.notifyHasMessage(targetMailbox, DefaultSelector.ANTI_MONOTONE);
        }

        if (monotoneQueue.isEmpty()) {
            targetGroup.notifyLostAllMessages(targetMailbox, DefaultSelector.MONOTONE);
        } else {
            targetGroup.notifyHasMessage(targetMailbox, DefaultSelector.MONOTONE);
        }
    }
        ...
        if (kind == DefaultSelector.ANTI_MONOTONE) {
            ...
            for (final Entry<Tuple, Integer> entry : this.antiMonotoneQueue.getTuples().entrySet()) {
                ...
                for (int i = 0; i < Math.abs(count); i++) {
                    this.receiver.update(Direction.REVOKE, update, DifferentialTimestamp.ZERO);
                }
            }
            ...
            for (final Entry<Tuple, Integer> entry : this.monotoneQueue.getTuples().entrySet()) {
                ...
                for (int i = 0; i < count; i++) {
                    this.receiver.update(Direction.INSERT, update, DifferentialTimestamp.ZERO);
                }
            }
