...
abstract class QueryPerformanceTest {
    ...
    public def queryPerformance() {
        ...
        for (IQuerySpecification<?> _specification : specifications) {
            current++
            debug(
                "Measuring query " + _specification.getFullyQualifiedName + "(" + current + "/" + numOfSpecifications +
                    ")")
            ...
            def performMeasurements(IQuerySpecification<?> _specification, int current, long usedHeapBefore) {
                ...
                val matcher = queryEngine.getMatcher(specification,
                    new QueryEvaluationHint(queryBackendFactory, newHashMap))
                watch.stop()
                ...
                val result = new QueryPerformanceData(current, countMatches, usedHeapBefore, usedHeapAfter, usedHeap,
                    elapsed)
