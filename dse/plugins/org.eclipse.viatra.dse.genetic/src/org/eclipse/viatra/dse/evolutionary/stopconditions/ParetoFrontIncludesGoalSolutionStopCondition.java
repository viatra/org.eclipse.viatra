...
public class ParetoFrontIncludesGoalSolutionStopCondition implements IStopCondition {
    ...
    public boolean checkStopCondition(Collection<TrajectoryFitness> survivedPopulation) {
        for (TrajectoryFitness trajectoryFitness : survivedPopulation) {
            if (trajectoryFitness.rank == 1 && trajectoryFitness.fitness.isSatisifiesHardObjectives()) {
                return true;
            }
        }
        return false;
